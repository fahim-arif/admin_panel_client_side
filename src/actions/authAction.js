...
export const Forgetpassword = (userData) => (dispatch) => {
  axios
    .post("/api/admin/forget", userData)
    .then((res) => {})
    .catch((err) =>
      dispatch({
        type: GET_ERRORS,
...
export const loginRegister = (userData) => (dispatch) => {
  axios
    .post("/api/store/register", userData)
    .then((res) => {
      // Save to LocalStorage
      const { token } = res.data;
      //Set Token to Localstorage
...
export const purchasePlan = (userData) => (dispatch) => {
  axios
    .post("/api/store/purchaseplan", userData)
    .then((res) => {
      // Save to LocalStorage
      const { token } = res.data;
      //Set Token to Localstorage
